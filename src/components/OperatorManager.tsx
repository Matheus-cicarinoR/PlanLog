  ...
  <div className="space-y-6 pb-12">
   ...
   <div className="flex flex-col sm:flex-row gap-4 mb-2">
    <div className="flex-1 max-w-sm space-y-1.5">
     <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Profissional / Operador</span>
     <div className="relative">
      <select
       value={selectedOperatorId || ''}
       onChange={(e) => setSelectedOperatorId(e.target.value || null)}
       className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-800 focus:outline-none focus:border-amber-500/50 appearance-none cursor-pointer"
      >
    ...
    <div className="flex items-end">
     <button
      ...
      className="flex items-center gap-2 px-4 h-[42px] rounded-xl border border-dashed border-slate-300 bg-transparent text-slate-500 hover:bg-slate-50 transition-all whitespace-nowrap"
     >
   ...
   {selectedOperator ? (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fadeIn">
     ...
     <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm space-y-5">
      <div className="flex items-center justify-between pb-4 border-b border-slate-100 ">
       <div className="flex items-center gap-3">
        ...
        <div>
         <h3 className="text-lg font-black text-slate-900 mb-0.5">{selectedOperator.nome}</h3>
       ...
       <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
         <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 ">
          <span className="text-[11px] text-slate-500 block mb-1">Telefone</span>
         ...
         <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 ">
          <span className="text-[11px] text-slate-500 block mb-1">Remuneração</span>
         ...
         {selectedOperator.chave_pix && (
          <div className="col-span-1 sm:col-span-2 p-3 rounded-xl bg-slate-50 border border-slate-100 ">
           <span className="text-[11px] text-slate-500 block mb-1">Chave PIX</span>
     ...
     {(() => {
      ...
      const colorClass = isFinanceiro ? "text-blue-600 " : "text-amber-600 ";
      const bgClass = isFinanceiro ? "bg-blue-50 border-blue-100 " : "bg-amber-50 border-amber-100 ";

      return (
       <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm space-y-5 flex flex-col">
        ...
        <div className="grid grid-cols-2 gap-3 shrink-0">
         <div className={`p-4 rounded-xl border ${bgClass}`}>
          <span className="text-xs text-slate-600 font-semibold block mb-1">Total Recebido (Dinheiro)</span>
          ...
          {servicosRecebidos.length === 0 ? (
           ...
           servicosRecebidos.map((s) => (
            <div key={s.id} className="p-3 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-between">
             <div>
              <span className="font-bold text-slate-900 text-sm block mb-0.5">{s.cliente}</span>
    ...
    <div className="bg-white rounded-2xl p-12 border border-slate-200 shadow-sm flex flex-col items-center justify-center text-center">
     ...
     <h3 className="text-lg font-bold text-slate-900 mb-1">Nenhum operador selecionado</h3>
     ...
     <button
      ...
      className="px-4 py-2 bg-slate-900 hover:bg-slate-800  text-white font-bold rounded-xl shadow-md shadow-slate-900/20 transition-transform active:scale-95 flex items-center gap-2 cursor-pointer"
     >
