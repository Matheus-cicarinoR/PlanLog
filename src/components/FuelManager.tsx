 ...
 return (
  <div className="space-y-6 pb-12">
   ...
   <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
    ...
    <div className="bg-white rounded-2xl p-3.5 sm:p-5 border border-slate-200 shadow-sm">
     <div className="flex items-center justify-between mb-2">
      <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500 truncate">Total Diesel</span>
      <div className="p-1.5 sm:p-2 rounded-xl bg-amber-50 text-amber-600 shrink-0">
       <Fuel className="w-4 h-4 sm:w-5 sm:h-5" />
      </div>
     </div>
     <div className="text-xl sm:text-2xl lg:text-3xl font-black text-slate-900 font-mono truncate">{totalLitros.toFixed(1)} L</div>
     <span className="text-[10px] sm:text-xs text-slate-500 mt-1 block truncate">Volume total</span>
    </div>

    <div className="bg-white rounded-2xl p-3.5 sm:p-5 border border-slate-200 shadow-sm">
     <div className="flex items-center justify-between mb-2">
      <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500 truncate">Custo Total</span>
      <div className="p-1.5 sm:p-2 rounded-xl bg-red-50 text-red-600 shrink-0">
       <DollarSign className="w-4 h-4 sm:w-5 sm:h-5" />
      </div>
     </div>
     <div className="text-xl sm:text-2xl lg:text-3xl font-black text-red-600 font-mono truncate">{formatCurrency(totalGasto)}</div>
     <span className="text-[10px] sm:text-xs text-slate-500 mt-1 block truncate">~R$ {(totalGasto / (totalLitros || 1)).toFixed(2)}/L</span>
    </div>

    <div className="bg-white rounded-2xl p-3.5 sm:p-5 border border-slate-200 shadow-sm">
     <div className="flex items-center justify-between mb-2">
      <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500 truncate">Consumo Médio</span>
      <div className="p-1.5 sm:p-2 rounded-xl bg-blue-50 text-blue-600 shrink-0">
       <Gauge className="w-4 h-4 sm:w-5 sm:h-5" />
      </div>
     </div>
     <div className="text-xl sm:text-2xl lg:text-3xl font-black text-blue-600 font-mono truncate">{consumoMedioHoras.toFixed(1)} L/h</div>
    ...
    <div className="bg-white rounded-2xl p-3.5 sm:p-5 border border-slate-200 shadow-sm">
     <div className="flex items-center justify-between mb-2">
      <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500 truncate">Custo / Hora</span>
      <div className="p-1.5 sm:p-2 rounded-xl bg-emerald-50 text-emerald-600 shrink-0">
       <DollarSign className="w-4 h-4 sm:w-5 sm:h-5" />
      </div>
     </div>
     <div className="text-lg sm:text-2xl lg:text-3xl font-black text-emerald-600 font-mono truncate">{formatCurrency(custoCombustivelPorHora)}/h</div>
     <span className="text-[10px] sm:text-xs text-slate-500 mt-1 block truncate">Impacto hora operada</span>
    </div>
   ...
   <div className="block md:hidden space-y-3">
    {paginatedAbastecimentos.map((abs) => (
     ...
     >
      ...
      <div className="grid grid-cols-3 gap-2 p-2 rounded-xl bg-slate-50 text-xs">
       <div>
        <span className="text-[10px] text-slate-400 block uppercase font-bold">Litros</span>
        <span className="font-mono font-black text-slate-900 ">{abs.litros.toFixed(1)} L</span>
       </div>
       <div>
        <span className="text-[10px] text-slate-400 block uppercase font-bold">Preço/L</span>
        <span className="font-mono text-slate-600 ">R$ {abs.preco_litro.toFixed(2)}</span>
       </div>
       <div>
        <span className="text-[10px] text-slate-400 block uppercase font-bold">Total</span>
        <span className="font-mono font-black text-emerald-600 ">{formatCurrency(abs.valor_total)}</span>
       </div>
      </div>
      ...
      <tbody className="divide-y divide-slate-100 ">
       {paginatedAbastecimentos.map((abs) => (
        <tr key={abs.id} className="hover:bg-slate-50/50 transition-colors">
         <td className="py-3 px-4 font-mono text-slate-800 ">{formatDate(abs.data)}</td>
         <td className="py-3 px-3 font-mono font-bold text-amber-600 ">{formatHours(abs.horimetro)}</td>
         <td className="py-3 px-3">
          <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-800 font-semibold text-xs border border-slate-200 ">
           {abs.tipo_combustivel}
          </span>
         </td>
         <td className="py-3 px-3 font-mono font-bold text-slate-900 ">{abs.litros.toFixed(1)} L</td>
         <td className="py-3 px-3 font-mono text-slate-500 ">R$ {abs.preco_litro.toFixed(2)}</td>
         <td className="py-3 px-3 font-mono font-bold text-emerald-600 ">{formatCurrency(abs.valor_total)}</td>
         <td className="py-3 px-3 text-slate-600 ">{abs.posto_fornecedor || 'Posto'}</td>
