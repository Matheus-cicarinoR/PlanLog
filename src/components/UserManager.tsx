  ...
  <div className="space-y-6 pb-12">
   ...
   <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
     <div className="flex items-center gap-2">
      <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-50 text-amber-600 border border-amber-500/30 flex items-center gap-1">
   ...
   <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <Card>
     <CardContent>
      <div className="flex items-center justify-between mb-2">
       <span className="text-xs font-bold uppercase tracking-wider text-slate-500 ">Total de Contas</span>
       <div className="p-2 rounded-xl bg-slate-100 text-slate-600 ">
        <User className="w-4 h-4" />
       </div>
      </div>
      <div className="text-3xl font-black text-slate-900 font-mono">{totalUsuarios}</div>
      <span className="text-xs text-emerald-600 mt-1 block font-semibold">{totalAtivos} ativos &bull; {totalUsuarios - totalAtivos} bloqueados</span>
     </CardContent>
    </Card>
    ...
    <Card>
     <CardContent>
      <div className="flex items-center justify-between mb-2">
       <span className="text-xs font-bold uppercase tracking-wider text-amber-600 ">Administradores</span>
       <div className="p-2 rounded-xl bg-amber-50 text-amber-600 ">
        <Shield className="w-4 h-4" />
       </div>
      </div>
      <div className="text-3xl font-black text-amber-600 font-mono">{totalAdmins}</div>
      <span className="text-xs text-slate-500 mt-1 block">Acesso total ao sistema</span>
     </CardContent>
    </Card>
    ...
    <Card>
     <CardContent>
      <div className="flex items-center justify-between mb-2">
       <span className="text-xs font-bold uppercase tracking-wider text-blue-600 ">Financeiro & Caixa</span>
       <div className="p-2 rounded-xl bg-blue-50 text-blue-600 ">
        <Key className="w-4 h-4" />
       </div>
      </div>
      <div className="text-3xl font-black text-blue-600 font-mono">{totalFinanceiros}</div>
      <span className="text-xs text-slate-500 mt-1 block">Controle de receitas e DRE</span>
     </CardContent>
    </Card>
    ...
    <Card>
     <CardContent>
      <div className="flex items-center justify-between mb-2">
       <span className="text-xs font-bold uppercase tracking-wider text-emerald-600 ">Operadores de Campo</span>
       <div className="p-2 rounded-xl bg-emerald-50 text-emerald-600 ">
        <UserCog className="w-4 h-4" />
       </div>
      </div>
      <div className="text-3xl font-black text-emerald-600 font-mono">{totalOperadores}</div>
      <span className="text-xs text-slate-500 mt-1 block">Lançamentos e telemetria</span>
     </CardContent>
    </Card>
      ...
      <tbody className="divide-y divide-slate-100 ">
       {filteredUsers.length === 0 ? (
        ...
        filteredUsers.map((u) => {
         ...
         return (
          <tr key={u.id} className={`hover:bg-slate-50/50 transition-colors ${isBlocked ? 'opacity-50 bg-red-50 ' : ''}`}>
           
           {/* Nome e Avatar */}
           <td className="py-3.5 px-4 font-bold text-slate-900 whitespace-nowrap">
            <div className="flex items-center gap-3">
             <div className="w-9 h-9 rounded-xl bg-slate-100 text-amber-600 border border-slate-200 flex items-center justify-center font-bold text-sm shadow-sm">
              {icon}
             </div>
             <div>
              <span className="text-slate-900 block font-bold">{u.nome}</span>
              {isMe && (
           ...
           {/* Último Acesso */}
           <td className="py-3.5 px-3 text-slate-500 font-mono text-xs whitespace-nowrap">
           ...
           <td className="py-3.5 px-4 text-right whitespace-nowrap">
            <div className="flex items-center justify-end gap-1.5">
             ...
             <button
              ...
              className={`p-2 rounded-xl border transition-colors ${
               isBlocked
                ...
                : 'bg-slate-100 hover:bg-amber-50 text-amber-600 border-slate-200 '
             ...
             <button
              ...
              className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-blue-600 border border-slate-200 transition-colors"
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col flex-1 overflow-hidden">
       ...
       <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
         ...
         <select
          {...register('cargo')}
          className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400"
         >
       ...
       <div className="flex items-center justify-end gap-3 px-5 py-3.5 sm:px-6 bg-slate-50 border-t border-slate-200 shrink-0">
        ...
        <Button
         ...
         className="bg-slate-900 hover:bg-slate-800  text-white font-black cursor-pointer shadow-md shadow-slate-900/20 "
        >
