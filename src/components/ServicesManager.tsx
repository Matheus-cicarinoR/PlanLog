...
export const ServicesManager: React.FC<ServicesManagerProps> = ({
 ...
 return (
  <div className="space-y-6 pb-12">
       ...
       <button
        ...
        className={`px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
         statusFilter === 'pendente'
          ...
          : 'bg-slate-50 text-red-600 hover:bg-red-50 border border-red-500/30'
        }`}
       >
       ...
       <button
        ...
        className={`px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
         statusFilter === 'parcial'
          ...
          : 'bg-slate-50 text-amber-600 hover:bg-amber-50 border border-amber-500/30'
        }`}
       >
       ...
       <button
        ...
        className={`px-3 py-2 rounded-lg text-xs font-semibold transition-all ${
         statusFilter === 'deslocamento'
          ...
          : 'bg-slate-50 text-blue-600 hover:bg-blue-50 border border-blue-500/30'
        }`}
       >
   ...
   <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-white p-3 rounded-xl border border-slate-200 shadow-sm text-xs">
    <div>
     <span className="text-slate-500 block">Horas Operadas:</span>
     <span className="font-bold text-slate-900 text-sm">{formatHours(totalHorasOperadasFiltro)}</span>
    ...
    <div>
     ...
     <span className="font-bold text-slate-900 text-sm">{formatCurrency(totalValorFiltro)}</span>
    </div>
    <div>
     <span className="text-slate-500 block">Total Recebido:</span>
     <span className="font-bold text-emerald-600 text-sm">{formatCurrency(totalPagoFiltro)}</span>
    </div>
    <div>
     <span className="text-slate-500 block">Saldo Pendente:</span>
     <span className="font-bold text-red-600 text-sm">{formatCurrency(totalDevedorFiltro)}</span>
    </div>
   </div>
     ...
     <div className="block md:hidden space-y-3">
      {paginatedServicos.map((servico) => {
       ...
       return (
        <div
         key={servico.id}
         className={`p-4 rounded-2xl bg-white border shadow-xs transition-all space-y-3 ${
          isPending
        ...
        >
          ...
          <div>
           ...
           <div className="text-xs text-slate-500 flex flex-wrap items-center gap-1.5 mt-0.5">
            ...
            {Number(servico.tempo_deslocamento_horas || 0) > 0 && (
             <span className="text-[10px] px-1.5 py-0.2 rounded bg-blue-50 text-blue-600 border border-blue-500/20 font-mono font-bold">
         ...
         {/* Financial Mini Grid */}
         <div className="grid grid-cols-2 gap-2 p-2.5 rounded-xl bg-slate-50 text-xs">
          <div>
          ...
          {isPartial && (
           <div className="col-span-2 text-red-600 font-bold font-mono text-[11px] pt-1 border-t border-slate-200 ">
            Falta pagar: {formatCurrency(servico.saldo_devedor)}
           </div>
          )}
          {servico.entregue_a && (
           <div className="col-span-2 text-[11px] text-blue-600 font-semibold">
         ...
         <div className="flex items-center justify-between gap-1.5 pt-2 border-t border-slate-100 ">
          ...
          <button
           ...
           className="px-2.5 py-1.5 rounded-xl bg-slate-100 text-slate-700 text-xs font-bold flex items-center gap-1 hover:bg-slate-200 cursor-pointer"
          >
          ...
          <button
           ...
           className="p-2 rounded-xl bg-emerald-50 text-emerald-600 hover:bg-emerald-100 cursor-pointer"
           title="WhatsApp"
          >
       ...
       <table className="w-full text-left border-collapse">
        ...
        <tbody className="divide-y divide-slate-100 text-xs sm:text-sm">
         {paginatedServicos.map((servico) => {
          ...
          if (isPending) {
           rowBg = 'bg-red-50/50 hover:bg-red-50 border-l-4 border-l-red-500';
          } else if (isPartial) {
           rowBg = 'bg-amber-50/50 hover:bg-amber-50 border-l-4 border-l-amber-500';
          }
           ...
           <tr key={servico.id} className={rowBg}>
            <td className="py-3.5 px-4 font-bold text-slate-900 whitespace-nowrap">
             <div className="flex items-center gap-2">
              {servico.tipo_registro === 'deslocamento_interno' ? (
               ...
               <span className={isPending ? 'text-red-600 font-extrabold' : ''}>
                {servico.cliente}
               </span>
              )}
             </div>
             {servico.descricao_servico && (
              <span className="text-xs text-slate-500 block font-normal truncate max-w-xs">
               {servico.descricao_servico}
              </span>
             )}
            </td>
            ...
            <td className="py-3.5 px-3 font-mono font-bold whitespace-nowrap">
             {servico.tipo_registro === 'deslocamento_interno' ? (
              ...
              <>
               <div className={isPending ? 'text-red-600 ' : 'text-slate-900 '}>
                {formatCurrency(servico.valor_total)}
               </div>
               {isPartial && (
            ...
            <td className="py-3.5 px-3 whitespace-nowrap">
             {servico.forma_pagamento === 'a_definir' ? (
              <span className="text-red-600 font-semibold text-xs">Em Aberto</span>
             ) : (
              <div>
            ...
            <td className="py-3.5 px-3 text-slate-600 ">
             {servico.entregue_a ? (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold bg-slate-100 text-blue-600 border border-slate-200 ">
            ...
            <td className="py-3.5 px-4 text-right whitespace-nowrap">
             <div className="flex items-center justify-end gap-1">
              ...
              <button
               ...
               className="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-emerald-600 transition-colors cursor-pointer"
               title="WhatsApp"
              >
