  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5 sm:gap-4">
        ...
        <div className="md:col-span-2 bg-white dark:bg-slate-900 rounded-2xl p-4 sm:p-5 border border-slate-200 dark:border-slate-800 shadow-sm space-y-3.5 sm:space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
            <div className="flex items-center gap-2.5">
              <div className="p-2 rounded-xl bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 shrink-0">
              ...
              <div>
                ...
                <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400">Intervalo recomendado: a cada {config.intervalo_troca_oleo_horas}h</p>
              </div>
            </div>

            <span className={`self-start sm:self-auto text-xs px-2.5 py-1 rounded-full font-bold font-mono ${
              horasRestantesOleo <= 25 ? 'bg-red-50 dark:bg-red-500/20 text-red-600 dark:text-red-400 border border-red-500/30' : 'bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30'
            }`}>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-[11px] sm:text-xs font-mono text-slate-500 dark:text-slate-400 flex-wrap gap-1">
              <span>Última: {formatHours(config.ultimo_oleo_horimetro)}</span>
              <span className="text-amber-600 dark:text-amber-400 font-bold">Atual: {formatHours(config.horimetro_atual)}</span>
          ...
          <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-200 dark:border-slate-800 text-[11px] sm:text-xs">
            <div>
              <span className="text-slate-500 dark:text-slate-400 block truncate">Vida Útil:</span>
              <span className="font-bold text-slate-800 dark:text-slate-200">{percentualOleoRestante}%</span>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 block truncate">Rodadas:</span>
              <span className="font-bold text-slate-800 dark:text-slate-200">{horasDesdeUltimoOleo.toFixed(1)}h</span>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 block truncate">Filtros:</span>
              <span className="font-bold text-amber-600 dark:text-amber-400 truncate block">Óleo, Comb., Ar</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 sm:p-5 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Total em Manutenção</span>
              <div className="p-2 rounded-xl bg-orange-50 dark:bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20">
                <DollarSign className="w-4 h-4 sm:w-5 sm:h-5" />
              </div>
            </div>
            <div className="text-2xl sm:text-3xl font-black text-orange-600 dark:text-orange-400 font-mono tracking-tight">
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-slate-800 mt-3 flex items-center justify-between text-xs">
            <span className="text-slate-500 dark:text-slate-400">Paradas Urgentes:</span>
      ...
      <div className="flex items-center gap-1.5 bg-white dark:bg-slate-900 p-2.5 sm:p-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-x-auto custom-scrollbar">
        {[
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {paginated.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-all shadow-sm flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div>
                      <span className="text-[10px] font-bold font-mono uppercase tracking-wider px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                        {item.tipo.replace('_', ' ')}
                      </span>
                    ...
                    <span className={`text-xs px-2.5 py-1 rounded-full font-bold flex items-center gap-1 border ${
                      isDone
                        ? 'bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border-emerald-500/30'
                        : isScheduled
                        ? 'bg-blue-50 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 border-blue-500/30'
                        : 'bg-red-50 dark:bg-red-500/20 text-red-600 dark:text-red-400 border-red-500/30'
                    }`}>
                  ...
                  {item.descricao_pecas && (
                    <p className="text-xs text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800 p-2.5 rounded-lg border border-slate-200 dark:border-slate-700 mb-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs text-slate-500 dark:text-slate-400">
                    <div>
                      <span className="text-slate-500 dark:text-slate-400 block">Horímetro:</span>
                      <span className="font-bold text-amber-600 dark:text-amber-400 font-mono">{formatHours(item.horimetro_momento)}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 dark:text-slate-400 block">Data:</span>
                      <span className="font-semibold text-slate-600 dark:text-slate-300">{formatDate(item.data_manutencao)}</span>
                    </div>
                    <div>
                      <span className="text-slate-500 dark:text-slate-400 block">Mecânico:</span>
                      <span className="font-semibold text-slate-600 dark:text-slate-300">{item.mecanico_responsavel || 'Oficina'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-slate-200 dark:border-slate-800 mt-4 flex items-center justify-between">
                  <div>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block">Custo Total</span>
                    <span className="text-lg font-black text-slate-900 dark:text-slate-100 font-mono">{formatCurrency(item.valor_total)}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-red-50 dark:hover:bg-red-900/30 text-slate-500 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 transition-colors"
                      title="Excluir"
                    >
