...
export class ErrorBoundary extends Component<Props, State> {
 ...
 public render() {
  if (this.state.hasError) {
   return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 sm:p-6 bg-[#e9eef5] text-slate-900 transition-colors">
     <div className="relative w-full max-w-xl bg-white/95 backdrop-blur-md border border-red-200 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 animate-scaleUp">
      
      {/* Header with Icon */}
      <div className="flex items-center gap-3.5">
       <div className="p-3 rounded-2xl bg-red-500/10 text-red-600 border border-red-500/30 shrink-0">
        <AlertOctagon className="w-8 h-8 stroke-[2.2]" />
       </div>
       <div>
        <span className="text-[11px] font-black uppercase tracking-wider text-red-600 font-mono">
      ...
      {/* Error Message Box */}
      {this.state.error && (
       <div className="p-3.5 rounded-2xl bg-slate-900 text-slate-100 border border-slate-800 text-xs font-mono overflow-hidden space-y-2">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-2.5 pt-2">
       ...
       <Button
        ...
        className="w-full sm:flex-1 font-bold border-slate-200 hover:bg-slate-100 rounded-xl py-2.5 text-xs sm:text-sm flex items-center justify-center gap-2 cursor-pointer"
       >
