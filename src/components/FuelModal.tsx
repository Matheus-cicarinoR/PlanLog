   ...
   <div className="relative w-full max-w-lg max-h-[92vh] flex flex-col bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden animate-scaleUp">
    ...
    <div className="flex items-center justify-between px-5 py-4 sm:px-6 bg-slate-50 border-b border-slate-200 shrink-0">
     <div className="flex items-center gap-2.5">
      <div className="p-2 rounded-xl bg-amber-50 text-amber-600 border border-amber-500/20">
       <Fuel className="w-5 h-5" />
      </div>
      <div>
       <h3 className="text-base sm:text-lg font-black text-slate-900 ">Registrar Abastecimento</h3>
       <p className="text-xs text-slate-500 ">Controle de combustível (Diesel) da retroescavadeira.</p>
      </div>
     </div>
     <button onClick={onClose} className="p-2 rounded-xl text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-colors cursor-pointer">
      <X className="w-5 h-5" />
     </button>
    </div>
    ...
    <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
     <div className="p-5 sm:p-6 space-y-4 overflow-y-auto custom-scrollbar flex-1 text-xs sm:text-sm">
      ...
      {/* Máquina Vinculada */}
      <div>
       <label className="block text-slate-600 font-semibold mb-1 flex items-center gap-1.5">
        <Tractor className="w-3.5 h-3.5 text-amber-500 " /> Máquina Vinculada *
       </label>
       <select
        value={maquinaId}
        onChange={(e) => setMaquinaId(e.target.value)}
        className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-colors cursor-pointer"
       >
        {maquinas.map((maq) => (
      ...
      <div className="grid grid-cols-2 gap-3">
       <div>
        ...
        <input
         ...
         className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-colors"
        />
       </div>
       <div>
        <label className="block text-slate-600 font-semibold mb-1 flex items-center gap-1">
         <Gauge className="w-3.5 h-3.5 text-slate-500 " /> Horímetro (h)
        </label>
        <input
         type="number"
         step="0.1"
         ...
         className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 font-mono focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-colors"
        />
       </div>
      </div>
      ...
      <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
       <div className="grid grid-cols-2 gap-3">
        <div>
         <label className="block text-slate-500 text-xs font-semibold mb-1">Litros de Diesel *</label>
         <input
          type="number"
          step="0.01"
          required
          ...
          className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 font-mono font-bold text-base focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-colors"
         />
        </div>
        <div>
         <label className="block text-slate-500 text-xs font-semibold mb-1">Preço por Litro (R$)</label>
         <input
          type="number"
          step="0.01"
          value={precoLitro}
       ...
       <div>
        <label className="block text-slate-500 text-xs font-semibold mb-1">Valor Total (R$) *</label>
        <input
         type="number"
         step="0.01"
         required
         ...
         className="w-full px-3 py-2 bg-white border border-amber-500/40 rounded-lg text-amber-600 font-mono font-bold text-lg focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-colors"
        />
       </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
       <div>
        <label className="block text-slate-600 font-semibold mb-1">Tipo de Combustível</label>
        <select
         value={tipoCombustivel}
         onChange={(e) => setTipoCombustivel(e.target.value as any)}
         className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-colors cursor-pointer"
        >
       ...
       <div>
        <label className="block text-slate-600 font-semibold mb-1">Posto / Fornecedor</label>
        <input
         type="text"
         value={postoFornecedor}
         onChange={(e) => setPostoFornecedor(e.target.value)}
         placeholder="Ex: Posto Trevo Petrobras"
         className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-colors placeholder-slate-400 "
        />
       </div>
      </div>
     ...
     {/* Footer (Fixed) */}
     <div className="flex items-center justify-end gap-3 px-5 py-3.5 sm:px-6 bg-slate-50 border-t border-slate-200 shrink-0">
      <button
       type="button"
       onClick={onClose}
