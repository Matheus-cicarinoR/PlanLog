 ...
 return (
  <Link
   ...
   className={`group flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm transition-all duration-200 ${
    isActive
     ? "bg-slate-100 text-slate-900 font-bold shadow-sm active"
  ...
  >
   <div className="flex items-center justify-between w-full">
     ...
     {item.icon ? (
      ...
      <span
       className={`rounded-full mx-1.5 h-2 w-2 shrink-0 ${
        isActive ? "bg-slate-950" : "bg-slate-300 group-hover:bg-slate-500"
       }`}
      />
     )}
    ...
    {item.isPro ? (
     <span className="py-0 px-2 text-[10px] bg-indigo-50 text-indigo-600  rounded-full font-bold">
      Pro
     </span>
    ) : null}
   </div>
